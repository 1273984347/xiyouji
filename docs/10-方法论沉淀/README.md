...
| 6 | E2 文档同步区分历史 vs 现役 | （待创建·详见 [user_profile](../../../.trae-cn/memory/user_profile.md)） | 文档同步时区分历史变更日志条目（保留旧值）和现役描述（必须更新为新值） |
...
| 8 | 并行 Edit 竞态问题 | （待创建·详见 [user_profile](../../../.trae-cn/memory/user_profile.md)） | 同一文件的多个 Edit 并行执行时后者覆盖前者·应串行执行或 replace_all |
