...
| 批次 | 状态 | commit |
|:---|:---|:---|
| W493 本批 | ✅ 完成 | 9543f31（v2.3.92） |

**证据完整性注记（W495 补）**：本批一次性修复脚本曾误清空 224 个 data+EN 页的 INLINED CSS 块（全站渲染裸文本），M5「内联 28385B」实测数字在提交时已失真（系沿用 W489 旧值）。W495 热修复（inline_css --force 恢复 + check_inlined_css.py 门禁转正）已恢复全部内联块（实测 30659B ≤33KB），M5 以 W495 实测为准。
