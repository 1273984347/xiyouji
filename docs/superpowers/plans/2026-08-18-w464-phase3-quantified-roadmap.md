...
> 版本：v6（定稿·回填二次评估 R6–R11 与口径修正）· 2026-08-18
...
- Q1：`python scripts/verify_delivery.py` 核心全绿（六文档同步 / 611 计数 / 105 学术轨引用 / A1 相邻性 / sitemap / CSP / 结构 / 语法 / 动态链接）。
...
- 量化验收：
  ...
  - 分析/RUM 脚本加载属性核实：`count.js` 为 async、`visit-log.js` 为 defer（抽查根/CN/EN 三类页面各 ≥ 1），不阻塞 LCP。
...
- 范围：**前置污染审计**（对比 push 时间戳与 UV 峰值相关性，剔除 CI 假访客后再判定）；按 1.2 阈值执行一次判定；**判定用 `scripts/judge_gate.py` 代码化执行**（输入来源定案：**GoatCounter API token，存 `.env`（已 gitignore）+ `judge_gate.py --fetch` 自动拉取**；备选人工录入 + 双人复核 + 录入元数据，二选一在 W464 定案）；输出 = 分发 / 归档 / 中间态；冲突时 GoatCounter 优先。
- 量化验收：报告表格含 5 项数字——7 日 UV、30 日 UV、UV 曲线、来源占比、跳出率；污染审计对照表（push 时间戳 × UV 峰值）；`judge_gate.py` 输出分支与报告记录一致；并记录「输入值 + 阈值 + 输出分支」，他人可复算同一结论。
- 产出：`docs/10-方法论沉淀/读者数据复盘.md`。
...
- 现状取证（2026-08-18 实测）：全站 234 页中仅 4 页含 meta description（curated / guide / index + 1 EN）、仅 index 含 JSON-LD——批量补齐 200+ 页，估算调至 **8–12h**。
- W467a · meta/OG 批量补齐：范围 = 全站 234 页（含 EN 138 页）+ sitemap/robots 一致性；实现 = 批量生成器 + 校验门禁（复用 docs_index.py 模式）；验收 = description 缺失页数 0、OG 缺失页数 0、sitemap 0 broken、robots.txt 不 Disallow 任何 sitemap URL。
- W467b · JSON-LD 结构化数据：范围 = 228 个 sitemap URL 全部覆盖；验收 = JSON-LD 覆盖 100%。
- 产出：SEO 健康报告（4 项数字）+ 补丁。

**W468 · S4 学术投稿候选**

- 范围：从 `source/引用与网络解读/学术论文索引.md` 中筛 2 篇选题，产出投稿稿。
- 选题准入（量化）：每篇选题须满足 ① 有 ≥ 3 篇学术论文索引支撑 ② 对应已有 A4/A5 内容。
- 量化验收：投稿稿 2 篇；每篇参考文献 100% 能回溯到学术论文索引条目；无死链。
- 产出：`docs/S4/`（学术投稿候选）2 篇。

**W469 · S2 精选分发增量**

- 范围：从 44 篇随笔 + 209 篇专题中续发精选；平台 ≥ 2 个（公众号/知乎/博客 三选 ≥2，W469 启动时定）。
...
- **R6（新增·高）CI 访问污染 GoatCounter 计数**：截图（file:// 仍执行 goatcounter.js）与 LHCI（localhost 5 页 × 3 次/push）每次 push 注入假访客；W465 判定前必须先做污染审计（push 时间戳 × UV 峰值对照），并在 CI 中拦截 count.js beacon（Playwright `route` abort）或 GoatCounter 侧按 UA/IP 排除。
...
python scripts/check_structure.py        # 0 结构失衡
