...
- **默认权限模式**：`bypassPermissions`（本地已知项目，便于直接运行分析脚本）。新对话时可在底部随时切回 `acceptEdits` / `default`。
...
- **切换项目**：新建对话时修改「工作目录」即可让 Agent 操作其他目录；或改 `.env` 的 `PROJECT_CWD`。

## 安全提示

默认 `bypassPermissions` 下，Agent 可在 `PROJECT_CWD` 内执行任意命令（含运行脚本、删改文件）。若处理重要数据，建议在新对话底部将权限模式切到 `acceptEdits` 或 `default`，对每一步工具调用手动确认。
