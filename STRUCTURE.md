...
> 当前版本：v2.3.67（2026-08-16）— W449 冗余文档清理（git rm 删除三冗余文档 + 依赖清理）·W448 外部锐评回应治理（STRUCTURE 归档 + 版本号语义说明 + AI 生成披露）— A1-A6 共 611 篇 + 86 可视化页（A4 209 篇 已含）·详细变更见 [CHANGELOG.md](CHANGELOG.md)。 + W450（统计口径统一与轨标体系） + W451（学术研究引用审计） + W452（学术研究显式引用门禁）
