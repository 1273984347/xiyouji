...
> 本文件记录项目前端视觉与交互规范，确保 86 个可视化页面（D3.js/Three.js）+ dashboard 入口在「古典宣纸风」基调下保持一致性，并兼容 v0.9.1 引入的 Name That UI 组件语言。
>
> 适用对象：所有 `site/` 下的 HTML 页面、后续新增可视化页、dashboard 迭代。
>
> **维护契约**：本文档是设计规范权威源，只允许结构性编辑——① 新增设计体系用「子节 / 字母后缀」（如 §4A），禁止打乱 §1–§11 既有编号；② 规范变更必须**就地修订**对应章节并更新 §11 相关文件；③ 批次/版本引用只作来源注记（如 §5 W460–W463），禁止堆批次历史（历史归 CHANGELOG）；④ 与 文档规范 / 交接文档 冲突时以文档规范为准。
