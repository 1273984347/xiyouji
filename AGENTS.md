...
- **batch_cascade.py 在 CRLF 文档上会翻倍行尾且头链双写**（W553）：级联落盘后 6 个 CRLF md 行尾变 4×CR（`\r\r\r\r\n`）、交接头链 entry 以「；」「·」各写一次——落盘后必须字节级核查（行尾 CR 串收敛 + 链唯一性断言），翻倍即 `rstrip(b'\r')` 修复。
