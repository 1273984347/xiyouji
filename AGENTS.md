# AGENTS.md — 《详解西游记》项目 Agent 指南

> 本文件面向进入本仓库工作的 AI Agent（及新接手的人类维护者），提供项目的用途、技术栈、目录结构、关键模块、依赖关系、构建运行方式与必须遵守的铁律。动手前请通读；更细的规则以文末「权威文档」为准。
>
> **维护契约（防乱写）**：本文档是 Agent 协作指南，只允许结构性编辑——① 保持 §1–§8 骨架不变；② 新增铁律/规则必须**就地并入对应章节**并全篇去重（禁止追加「速记 / 补充」尾部段落）；③ 版本脚注更新须与当前 HEAD（vX.Y.Z W###，用 `git rev-parse --short HEAD` 核对）一致；④ 与 交接文档 / 文档规范 / CHANGELOG 冲突时以后三者为准。

---

## 1. 项目用途与定位

**《详解西游记》**（xiyouji）是一个「一源多形 · 数字人文可视化解读《西游记》100 回」的项目。它把《西游记》的文本研究组织为四层产物：

- **文档（Docs）**：Markdown 写就的逐回解读、人物分析、主题专题、文化背景、诗词赏析、个人随笔。
- **站点（Site）**：可浏览的纯静态 HTML 站点，`file://` 双击即可打开，部署于 GitHub Pages（https://1273984347.github.io/xiyouji/ ）。
- **数据可视化**：86 个 D3.js / Three.js 可视化页（site/data/ 共 87 个 HTML，「86」不含模板壳 _shell.html），覆盖 133 个数据维度（章节统计、人物关系网络、八十一难热力图、取经路线、情感热力图、AI 对话等 34 类主题 A–AH）。
- **可问询入口**：Web Agent「西游记·渡口问津」（xiyouji-agent-web/）——基于 CodeBuddy Agent SDK，可对话、检索 docs/source、跑脚本。

**核心内容规模**：A1–A6 内容板块共 615 篇（A1 逐回 100 / A2 随笔 44 / A3 人物 215 / A4 主题 209 / A5 文化 34 / A6 诗词 13；算法与边界见 docs/00-导读/统计口径说明.md；W505 起 611→615）；英文站 site/en/ 138 页已全量英文化。

**设计方向**：锁定「新中式·数字雅集」（宣纸底/墨文/朱砂点缀/零外域依赖）；**页面动效是强制契约**——时长三档预算（≤150/250/600ms）、reduced-motion 双守卫、tooltip 统一 `.chart-tooltip`、count-up fail-open 等，详见 DESIGN.md（视觉 §1-4、**动效 §5**）。

**版本号语义**：`vX.Y.Z` 是**内容发布批次编号，不适用 SemVer**。每个发布批次有唯一 W### ID；判断"改了什么"看 CHANGELOG.md，不要从版本号推断兼容性。

---

## 2. 技术栈

| 层 | 技术 |
|---|---|
| 可视化前端 | D3.js v7（本地化 `site/static/js/d3.v7.min.js`）、Three.js r128（`three.r128.min.js`，手动 OrbitControls） |
| 站点 | 原生 HTML/CSS/JS，纯静态，file:// 直开，内联 tokens.css + system.css |
| 文本分析 | Python（词频/共现/情感/术语 NLP，stdlib + 少量依赖，见 scripts/requirements.txt） |
| 代码规范 | Ruff（pyproject.toml，line-length 120，py311）|
| Web Agent | React 18 + Vite 5 + TypeScript + TDesign React + Express 4 + better-sqlite3 + `@tencent-ai/agent-sdk`（CodeBuddy Agent SDK） |
| 测试 | pytest + Playwright E2E（冒烟/交互/视觉回归） |
| CI/CD | GitHub Actions（pytest / Playwright / Lighthouse / pip-audit / npm-audit / Pages 部署 / 截图审查） |

**运行时要求**：Node ≥ 20（Web Agent）；Python 3.11+（脚本）。

---

## 3. 目录结构

```
xiyouji/
├── docs/                  # Markdown 文档主体（内容真载体）
│   ├── 00-导读/           # 项目说明、文档规范、术语表、统计口径
│   ├── 01-全书逐回解读/   # 100 回逐回解读（A1）
│   ├── 02-人物深度分析/   # A3 人物谱系
│   ├── 03-主题与情节专题/ # A4 主题专题（209 篇）
│   ├── 04-文化与历史背景/ # A5 文化背景
│   ├── 05-诗词歌赋/       # A6 诗词
│   ├── 06-个人随笔/       # A2 随笔（44 篇）
│   ├── 07-09/             # 学以致用 / 提升认知 / 精神塑造
│   ├── 10-方法论沉淀/     # 复盘、诊断 SOP、DRL 真循环等可复利经验
│   ├── S2/S3/S4/          # 外部分享 / 方法论外部分享 / 学术投稿候选
│   ├── superpowers/       # 开发过程 spec/plan 档案
│   ├── _dev/  _templates/ # 开发内部文档 / 内容模板（勿直接套用）
│   └── INDEX.md           # 文档索引（docs_index.py 生成）
├── source/                # 原著原文（分回）+ 引用与网络解读（含学术论文索引）
├── site/                  # 可浏览 HTML 站点
│   ├── index.html dashboard.html dukou-engine.html curated.html 等
│   ├── data/              # 可视化页（87 个 HTML，D3/Three；「86 可视化页」口径不含模板壳 _shell.html）
│   ├── en/                # 英文站（138 页）
│   ├── static/js|css|fonts/  # 本地化 D3/Three、tokens.css/system.css、子集化字体
│   └── sitemap.xml
├── scripts/               # Python/JS 工具链（门禁、生成器、审计、诊断）
│   ├── verify_delivery.py # 交付门禁（pre-commit 强制，见 §5）
│   ├── generate_csp.py / inline_css.py / bump_version.py / lint_links.py
│   ├── check_*.py|js      # 各专项门禁
│   ├── run_all.py         # 批量运行 A-AH 34 类分析
│   ├── _*.py|js           # 一次性/诊断脚本（不入库门禁、不参与 CI）
│   └── output/            # data/*.json（可视化数据源）+ file-index.md + screenshots/
├── dataset/               # 结构化 JSON（可视化/API 数据源）
├── xiyouji-agent-web/     # Web Agent「西游记·渡口问津」（React+Vite+Express）
├── mcp-server/            # MCP 工具（xiyouji_mcp.py：drl_spotcheck 等）
├── tests/                 # pytest + Playwright E2E
├── assets/                # 字体源、图片
├── references/  timeline/  tools/  hyperframes/
├── README.md / STRUCTURE.md / CHANGELOG.md / DESIGN.md / 交接文档.md / 新Agent启动Prompt.md
└── LICENSE + LICENSE-CONTENT.md  # 双协议
```

---

## 4. 关键模块与依赖关系

### 4.1 四层架构（数据流）

```
source/（原著+引用）──生成──► docs/（解读文档）
                                │
docs/ ──分析/抽取──► scripts/output/data/*.json ──► site/data/*.html（可视化页，fetch JSON + EMBEDDED 回退）
                                │
docs/ ──渲染──► site/（导航/索引页，直接链 docs）
```

- **单一事实源**：设计令牌 `site/tokens.css` + 组件层 `site/system.css`，经 `scripts/inline_css.py` 内联进各 HTML（页面自包含、file:// 直开）。
- **数据回退铁律**：所有 site/data 可视化页必须内嵌 `EMBEDDED_DATA`/`EMBEDDED` 回退，fetch 失败时仍可渲染（file:// 双击可用）。

### 4.2 交付门禁体系（verify_delivery.py，pre-commit 强制）

`scripts/verify_delivery.py` 是提交前唯一硬门禁入口（`.git/hooks/pre-commit` → 本脚本，核心 FAIL 返回非 0 阻断）。内含：

1. **六文档同步**（核心 2 硬门禁：CHANGELOG.md + 交接文档.md 必须含当前 v/W——期望版本由 verify_delivery 动态取自 CHANGELOG 现役版段，dukou-engine.html 页脚降级为新鲜度 WARN（W518 起）；辅助 4 仅 WARN：README / STRUCTURE / 项目说明 / file-index）
2. **范围漂移检测**（html 最高 W ≤ 文档+归档最高 W）
3. **A4 计数一致（EXPECT_A4 字面量「209 篇」）** + **A1-A6 真实计数（六板块顶层 .md 磁盘计数 == README「共 N 篇」声明，当前 N=615）**
4. **学术研究轨显式引用**（105 篇）
5. **A1 导航相邻性**（上一回=N-1·下一回=N+1）
6. **docs/01 链接校验**（lint_links 0 broken）
7. **sitemap 覆盖**、**site/data 回退模式**
8. **CSP 漂移**（generate_csp.py --check，233 页哈希 0 漂移）
9. **数据漂移**（check_data_drift.js；W563 扩展：site/data/json 部署副本逐字节对账 47 副本基线 + 页面副本引用存在性 + 副本路径形态解析回原件 + EMBEDDED 单源页页名↔同名原件回退——对账面 44→46 页/71→74 项）、**腐蚀/插件**（check_corruption.py）
10. **内联脚本语法**（check_js_syntax.js，node vm.Script 批量编译，含 EN 站）
11. **CSS 结构平衡**（check_structure.py，括号/引号/url 闭合）
12. **token 覆盖率**（check_token_coverage.py，W493 转正：页面私有 `<style>` 块 UI 裸色——无豁免注释页必须 0、带 `e-track-exempt` 注释页 ≤ 登记 N；真裸 box-shadow 必须 0；INLINED 副本跳过，私有块识别靠 `tokens.css —` 注释特征）
13. **动效禁止清单**（check_motion_ban.py，W493 转正：cubic-bezier 负值 / 360° 旋转 / animation infinite / parallax，白名单仅 .chart-loading/.chart-fade-in）
14. **a11y 对比度**（a11y_audit.py E2-2，W493 挂载：P0+P1=0 阻断，WCAG 2.2 AA 正文 ≥4.5:1）
15. **INLINED CSS 完整性**（check_inlined_css.py，W495 转正：data/EN 页 INLINED 标记块内容 ≥20KB——防 W493 式「224 页内联块被清空而 14 门禁全绿漏网」回归）
16. **Skills 索引一致性**（W498 挂载·W562 退役：skills/ 目录经用户确认整体删除，门禁与 check_skills_index.py/sync_skills.py 同步退役，编号保留；4 个通用会话技能的用户级副本不受影响）
17. **索引健康**（check_index_health.py，W500 转正：file-index 段完整性/唯一性/最新段残留（豁免 W449-W463 损坏区·仅防新增）+ 方法论 README 双向覆盖（目录 md ↔ 索引链接·待创建占位 0）+ CHANGELOG 编号规则段上限 == 最新 W 段 + 治理文档引用一致性（文档规范.md scripts 引用存在性·verify 挂载脚本存在性）；W526 扩展：段倒序断言（豁免区外 W 号严格递减·防尾部追加）+ 段缺失检测（CHANGELOG 现役版段须均有 file-index 登记段））
18. **元信息块 v2**（check_frontmatter.py，W501 挂载：内容可信度轨——新文件（不在基线清单内）必须含血缘 + 核验状态 4 字段：生成来源（skill@commit 或 人工撰写）/生成模型（含「未记录」合法·禁编造）/生成日期 YYYY-MM-DD/核验状态三值枚举；基线 frontmatter-baseline.txt 冻结存量 611 篇豁免；口径见文档规范 §4.6）
19. **术语一致性**（check_glossary.py，W502 挂载：术语表↔dataset/glossary.json 双向同步（--generate 解析生成·禁手改）+ 人物称谓组规范词锚定（传递归一 圣僧→唐僧→玄奘·复合词掩码 心猿意马/金公木母黄婆）；基线 glossary-baseline.txt 冻结存量 303 篇 383 条豁免·只拦新增；口径见文档规范 §4.7）
20. **原著引文硬验证**（check_citations.py，W503 挂载：`> 原文引文（第N回）：“…”` 行必须对 dataset/text-search.json 去空白归一后精确子串命中·防 AI 幻觉引文；--dir docs 全量·任何未命中 = FAIL·存量引文行 = 0 无豁免；语法见文档规范 §4.8）
21. **动态链接门禁**（check_dynamic_links.py，W459 挂载：解析页面 JS 内拼接生成的链接目标（A1_DOC_MAP 映射、EN source_doc 等）并对仓库路径存在性断言，补 lint_links 静态盲区，死链 = FAIL 阻断提交）
22. **治理文档维护契约**（check_governance_docs.py，挂载即生效：七项检查扫描六文档——历史段遭改写 / 现役值残留多份 / 尾部追加速记段 / 「最后更新」新鲜度（W518 挂载：头部滚动链与尾页脚历史链两处链首条目均须 == CHANGELOG 现役段·finditer 全量扫描），命中记 WARN 不阻断提交（起步策略），为后续转 FAIL 积累监测数据）
23. **旁文档同步**（W526 挂载：.github/workflows/README.md 头部版本行（vX.Y.Z W###）== CHANGELOG 现役段 + 里程碑行 W450-W### 上限 == 现役 W，任一不符 FAIL——W499/W525 两次实证旁文档滞后漏网，verify 覆盖外盲区封堵）
24. **图表静态自洽**（W551 挂载：check_chart_data.py——R1 调用 d3.sankey( 必须引用 d3-sankey.min.js 且磁盘存在；R2 可见饼图系措辞（饼图/环形图/圆环图/Pie/Donut）×treemap 实现×无弧形实现 = 错配。源自 W550 截图审查实证的 10 页桑基空渲染与 4 组饼图错配；需真实渲染的遮挡/空带/溢出/桑基运行时由 scripts/check_screenshot_gates.js 动态门禁覆盖（挂 screenshot-review workflow，pageerror/sankey-typeof/溢出/reveal/画布遮挡五类））
25. **数据内容一致性**（W555 挂载，经用户确认：check_content_consistency.py --gate 第 25 门禁——W550 实证三类同页数字矛盾规则化：同实体对/同主语多值、图注数 vs 数组字面长、桑基去重节点数、案例数 vs 边数；--self-test 负样本 4/4；基线 content-consistency-baseline.txt 冻结 W554 人工裁决 4 条误报·只拦新增；L2 站-dataset 对账与 L3 语义判读为报告型见 W554 审计报告）

### 4.3 脚本工具链要点

- **改任何内联脚本后，必须重跑 `python scripts/generate_csp.py`**——否则 CSP sha256 哈希失配，整个内联脚本被浏览器拒执行（症状：无 pageerror 但内容空白、`window.__data` 未设置）。
- **批量正则改 CSS/JS 后必须验证括号/引号平衡**（`check_structure.py` / `_find_css_breaks.py`）。
- **CHANGELOG/方案档验收数字必须当批现测**（W496 铁律）：写 M 指标/内联字节数等数字前先跑 `python scripts/acceptance_snapshot.py` 从输出抄，禁止从上一批 CHANGELOG 复制（W493「28385B」跨批复制曾掩盖全站 INLINED 清空事故）。
- **bump_version.py** 一键补齐辅助 4 文档版本行（W576 起恢复可用：W536 写路径守卫根目录曾误算为 scripts/ 致任何调用必误判越界，经用户批准修复为项目根——与 W550 修复的 generate_csp 同款；其「全文件 W001-W### 范围替换」覆盖 batch_cascade 不触及的 README 目录树/交接 CHANGELOG 范围行等存量漂移面，整文件 text-mode 重写兼带孤立 \r 损伤归一）；**三个已知坑（①②多次复现 W524 扩证·③W528 实证）**：① 全局替换会污染 file-index 历史段（生成「WXXX WXXX」重复标题的空壳段、登记表为空，需手工补全当批文件表）；② `--desc` 与 `--note` **均会**往 STRUCTURE.md 头部 / docs/00-导读/项目说明.md 的上一版本行**追加**「 + WXYY(...)」而非替换主描述（W523 实测 `--note` 同样触发——此前仅登记 `--desc` 单触发面）；③ 三简单页脚（index/cross-time-danmaku/tag-cloud）同步为替换式、只匹配"链首 == 解析版本"的条目，页脚一旦滞后就永不被命中（W528 实证停在 W453 脱节 74 批）——跑完必须 Grep 校验版本行三处 + 三页脚版本实际前进并手工净化。
- **CHANGELOG 多版段插入/重排只用脚本 + 结构断言**（锚点唯一性 + 版段 order 校验），手工 Edit 大段会吞相邻版段标题；**新批版本号先 Grep 现役段取 max+1 再写**（防撞号）。
- **批量改 md 一律 Python `open(encoding='utf-8')`，禁用 PowerShell `Set-Content`**（后者默认写 UTF-8 BOM，项目 W427 曾全库清 BOM；`Add-Content` 同理慎用）。
- **同文件多个 Edit 必须串行执行**（并行 Edit 基于同一原始内容计算 diff，后写覆盖先写——W505 复现 4 次：金角银角「唐僧肉」/CHANGELOG W506 段/项目说明头部/交接文档标题均丢失）；并行批次后必须 Grep 验证所有修改点。
- **写 `> 原文引文` 行前先跑 `python scripts/_cite_probe.py --kw <关键词> --chap <回目>`** 从 text-search.json 提取候选，禁止凭记忆编造（W503 门禁只拦截"写入后未命中"，无法拦截"恰好像原文的编造"）；新文档（非基线豁免）用变体称谓（行者/唐僧/圣僧等）必须同时含 canonical（孙悟空/玄奘），写作前 Grep `dataset/glossary.json` 称谓组。
- **Windows 多行字符串/heredoc 一律走 Write 临时文件 + 命令 `-F` 参数 + Delete 临时文件**，禁用 `cat <<'EOF'` heredoc（PowerShell 不支持，报错；W514 git commit -F 中文提交信息二度复用教训）。
- **跨 session / 涉及远端状态**：执行前先 `git log --oneline -5` + `gh run list` 确认最新状态，识别并行交付真实增量（E41）；workflow 触发条件必须匹配真实开发流、门禁建置即绿、引用文件先 `git ls-files` 确认（E36-E42 CI 类，详见交接文档「三」W516 段）。
- **共享机制必须写入仓库内文件（git tracked）**：任何要让其他 Agent 读到的机制/规则/经验，一律写仓库内公共载体（AGENTS.md / 交接文档 / skills/ / docs/10-方法论沉淀/），**禁止只写全局路径**（如 `c:\Users\...\.trae-cn\skills\`）——其他 Agent 在项目 session 读的是仓库版，且 `sync_skills.py --sync` 为仓库→全局单向（全局修改会被覆盖）。改 skills/ 下任何 skill 必须改仓库版后 `python scripts/sync_skills.py --sync` 同步全局（W516 教训：上移机制曾误写全局版，被用户指出载体错误）。（W562：skills/ 目录已退役，本条 sync 指引仅存历史——用户级 `~/.zcode/skills/` 的 4 个通用会话技能为 junction，指向 `D:\open-source\`，与仓库无关。）
- **workflow/脚本用 git diff 输出做路径分类必须加 `-c core.quotePath=false`**（W523）：git 默认把非 ASCII 路径输出为带双引号的八进制转义串（交接文档.md → `"\344\272\244..."`），首尾引号破坏 bash case/glob 匹配（连 `*.md`、`docs/*` 都失配）→ 免审/定向分支被静默架空、恒走兜底全量（Screenshot Review scope 步骤曾因此每次推送 ~11min 全量截图）；验证靠本地 Git Bash 复刻分类逻辑对真实范围双向模拟。
- **W537 三新规（W536 批次声明失真复盘）**：① CHANGELOG「验证」栏必须以当批实跑 `verify_delivery` 的输出为准填写，「状态」栏仅在提交/推送实际落地后为真——自制批量文档脚本绕过 bump_version 防护时同样适用（W536 实证：声明先于验证 → 4 项 FAIL 状态下写出「全绿 + 已推送」）；② 版本行/页脚更新须整行替换并同步改描述文本——第 22/23 门禁只校验 W 号不校验描述（三简单页脚「W536 号挂 W535 描述」串号实证）；③ CHANGELOG「文件」清单中的新建文件必须 `git add`——无门禁覆盖「清单 ⊆ tracked」，漏 add 即静默丢失；④ JS/工具脚本改动推送前必须以真实参数冒烟一次——node --check 不覆盖运行时（W537/W538 白名单形状回归实证；batch_cascade.py 冒烟口径同此）。
- `_` 前缀脚本为一次性/诊断脚本，不入库门禁、不参与 CI。
- **批量关键字扫描/门禁必须穷尽同义变体**（W550）：可视化缺陷扫描的关键字要覆盖同义写法（如 饼图/环形图/圆环图/Pie Chart/Donut Chart）——「圆环图」变体曾致漏网；命中后以「可见措辞 × 实现签名（d3.arc/pie/treemap 计数）」双条件定错配，禁止单关键字定性。
- **SVG 标签量测四坑**（W553）：① `getBBox()` 是元素局部坐标系，跨 `<g>` 比较文本 bbox = 幻影重叠（W550 audit-contentavoid 曾据此单页 display:none 65 个未重叠标签——「行标签缺失」类缺陷根因是补丁遮蔽非布局），跨组比较必须 `getBoundingClientRect()`；② tspan 显式 `x=0` 在「x 属性定位」的 text 中是 g 原点绝对坐标，随节点定位的 text 折行须让 tspan 显式继承同一 x；③ d3 `selection.text()` 覆写 textContent 会抹掉子元素——先 text() 后 append('title')；④ 行宽量测禁用 canvas.measureText 字体串拼装（比真实渲染窄约 9%），用 svg 内隐藏探针 text 的 `getComputedTextLength()`。修复图表 svg 挂 `data-audit-skip` 即可让隐藏型 audit 补丁（axisfix4/contentavoid/labelavoid/labelavoid-all）在 svg 级循环跳过（旋转型 axisfix1/2/3 不识别该属性，维持 W550 已验收外观）。W557 全站复审实证该类缺陷在其余 15 页持续发作（页面级豁免≠类绝迹），已全局根治：20 页 contentavoid 补丁内 `getBBox()`→`getBoundingClientRect()`（探针验证行标签复活），data-audit-skip 保留为页面级豁免机制。
- **batch_cascade.py 在 CRLF 文档上会翻倍行尾且头链双写**（W553，W554 二犯实锤）：级联落盘后 6 个 CRLF md 行尾变 4×CR（`\r\r\r\r\n`）、交接头链 entry 以「；」「·」各写一次——落盘后必须字节级核查（行尾 CR 串收敛 + 链唯一性断言），翻倍即逐行 `rstrip(b'\r')` 收敛修复；断言口径见 `scripts/_w553_jiacheck.py`（改批号复用）。
- **标记与 CSS 演化脱节查「基类是否还在」**（W557）：全站复审 30+ 页同现「KPI 统计条纵向裸排」，根因是页面只用 `.kpi-card.alt-*` 变体而基类 `.kpi-row/.kpi-card` 布局规则在历史批次丢失——「同文案多页同病」信号应立即查共享 CSS 基类；修法为对缺失页注入基类（纯 token 引用，禁裸色回退防 token 覆盖率门禁）。
- **CSS 缺分号 = 整条声明静默丢弃**（W558）：`: var(--xxx)` 后无分号直接换行跟下一属性，CSS 解析把两条并为一条非法声明整体丢弃（每处丢 2 个属性）——全站 130 页 1006 处实测（ethics-consumption 21 处/narrative-experiment 22 处恰为视觉 FAIL 重灾页）；机械修复用 `: var(--x)
 prop:` 形态正则补分号（_w558_fix_semicolons.py）。与基类丢失同属「属性静默丢失」族，排查视觉缺陷时优先扫描。
- **fullPage 截图不触发 IntersectionObserver**（W554）：凡含 `.reveal-in`（js-reveal 滚动显现）的页面，截图/审查前必须逐屏滚动穿透（步进 innerHeight、回顶后等待），否则下方区块渲染为空白——W550 G4 教训的截图侧二犯（试审判 curated「整屏空白 FAIL」实为伪影）；已固化进 `scripts/_w554_review.js`。
- **EMBEDDED 回退铁律的部署态反例**（W554）：en/81-hardships.html EMBEDDED 清单为空数组 + fetch 指向 site 外路径（`../../scripts/output/data/`）——file:// 与 Pages 部署态双断（截图实锤 0/0 trials）。新增页面自查两条：EMBEDDED 必须含完整数据（不允许空数组等 fetch）；fetch 相对路径不得越出 site/（部署根=site/）。
- **W 批次收尾七步清单**（W543-W548 六批 dogfood 后 W563-W566 四批延续实证，沉淀自 2026-09-06 报告 S-A）：① 内容改动 → 专项门禁（改内联脚本/样式必跑 generate_csp + --check）；② 新增/修改 Python 脚本 → `python -m ruff check scripts/`（推送前本地预检，CI 红灯历史三类均为本地可拦截）；③ verify_delivery 核心全绿；④ batch_cascade dry-run → apply（自检通过即登记，无人工修复步骤）；⑤ Write 临时提交信息文件 + `git commit -F`（禁 heredoc）；⑥ `git push` → `gh run list` 确认（CI 与 Screenshot Review 为准）；⑦ 记忆/交接文档状态同步。
- **取证枚举三陷阱**（W563-W566 三犯沉淀）：① 站点/全仓级枚举一律 Python `glob(recursive=True)`，禁 shell 单层 glob（顶层扫描曾漏 scripts/B_人物/ 子目录生成器）；② `grep -n` 行号前缀输出不得与其他命令输出混读（行号曾被误读为 git ls-files 内容致 F17 假阳性方案项）；③ 判定 ugrep 循环失真时改用 Python 重扫（同会话三犯）。
- **部署态冒烟与 file:// 审查是两条独立覆盖面**（W565 实证，P04）：EMBEDDED 回退机制使 fetch 成功（部署）与 fetch 失败（file://）走上不同代码路径，character-appearance 契约失配在生产崩溃数日而 file:// 截图审查全绿。凡改动「数据装载层 / fetch 链路」的批次，收尾必跑 `scripts/_deploy_smoke.js`（http.server + Playwright 断言 4xx/pageerror）。
- ** Express 5 / Node 20+ 的 req 'close' 语义陷阱**（W568）：`req.on("close")` 在请求体读取完毕即触发（并非连接断开），SSE 路由据此提前 `res.end()` 会吞掉后续全部事件（实证：init 之后 error 事件整段丢失、前端「思考中」永久挂起）；SSE 断开清理改挂 `res.on("close")`（已完成）。副作用教训：P2-3 类「断开清理」修复在运行时大版本升级后须复核语义。
- **全局批量规则前置「设计惯例三查」**（W571）：对 tokens/system 级全局 CSS/JS 规则或正则批量修复，落规则前必查——① 页内注入机制盘点（style 块/id 标记，如 #audit-halo 光晕）；② 页内双轨设计（码/标签、光晕、fill=none）；③ 规则带豁免条件（如 :not(:has(#audit-halo)))+ 实拍复验。实证：W569 v2 文字反白一刀切误伤 168 光晕页（墨字+白晕本双主题可读，W571 :has 豁免修正）。
- **改 tokens.css/system.css 后必跑 `python scripts/inline_css.py --force` 分发**（W571）：与 generate_csp 同级必要——精修 tokens 规则后漏跑，:has 豁免只存在于源、未分发到 226 页内联副本（hardship-heatmap tick 实测 #F2EBDC 揭示）；分发后须实拍/样式探针复验。
- **d3.select 静态快照空选区族**（W575）：`d3.select('#x')` 在元素声明前执行、元素根本不存在、或把选区方法调在自建门面对象（仅 show/hide）上，均为永久空选区/静默 no-op——tooltip 类功能**桌面鼠标同样失效**且常无报错（一报错反而是 `node().offsetWidth` 型 TypeError）；修法三态=惰性取用/按 W460 形态补元素（必须置于捕获脚本之前）/门面补齐缺失方法。静态扫描「select 行号 vs 元素 id 行号」即可穷尽（W575 四例三型，81-hardships/ecology/global-pattern/cross-time-danmaku）。
- **触屏 tooltip e2e 三坑**（W575）：① tooltip 显隐多带 CSS transition，合成派发后同步读 `getComputedStyle().opacity` 恒为过渡起始值——探针以「预清空 tip 内容→派发→内容非空」同步信号判定，并 stash/restore 防污染页面；快照键不可含显隐态 class（visible 翻转致键漂移跨快照失配），用 id 或顶层序号；② Chrome 在 touchend 后把 hover 同步回真实鼠标位置并补发 mouseout——e2e 中一旦先动过真实鼠标，后续 tap 的 compat mouseout 会杀掉 tooltip 显示态（浏览器行为非页面缺陷），触屏断言序列须全部先于鼠标动作完成；③ 泛选择器 `[class*="tip"]` 会误伤 `small-multiple`（子串撞车）与 tooltip 内层元素（容器 opacity:0 隐藏时内层 computed opacity 仍 1）——精确化（tooltip 子串/tip 结尾）+ 顶层容器过滤；常显换内容型面板（prismTooltip 类）无隐藏态，隐藏断言以「触屏≡鼠标」经验对照替代硬编码 hidden 期望；④（W581）d3.drag 的 touchstarted 会 stopImmediatePropagation——document 冒泡层 touch 委托收不到可拖拽节点的触摸事件，注入须挂 document 捕获阶段（capture:true·先于目标阶段·免疫下游拦截）；⑤（W581）touchscreen.tap 的 tooltip 显隐在 Playwright 由 compat 事件随机承载（注入派发与断言错位·tap 式 PASS 可能是 compat 代劳）——断言改合成 TouchEvent 与取点同 tick 派发（兼顾力导向节点漂移），并以事件流取证甄别派发真身。

### 4.4 Web Agent（xiyouji-agent-web/）

- 后端 `server/index.ts`（Express + SSE 流式 + SQLite `data/chat.db`），前端 `src/`（React18 + Vite5 + TDesign）。
- 凭证 `CODEBUDDY_API_KEY`（`.env`，由 `.env.example` 复制，已 gitignore）。
- 默认 `PROJECT_CWD`=仓库根（启动时向上探测 AGENTS.md+site/tokens.css 自动解析·env 可覆盖·W596）；专属 Agent「渡口问津」内置项目 sysprompt；权限默认 `default`（UI 可切 `acceptEdits`/`plan`；`bypassPermissions` 仅当服务端 env `AGENT_WEB_ALLOW_BYPASS=1` 时放行，否则降级 default——W411 加固，W537 文档校正）。
- 核心入口：`server/index.ts`、`src/hooks/useAgents.ts`（DEFAULT_AGENT）、`src/config.ts`（主题色 #c8463a「西」Logo）、`src/pages/ChatPage.tsx`。

## 5. 构建与运行方式

### 5.1 统一编排（Makefile）

```bash
make help          # 查看全部目标
make analyze       # 批量运行 A-AH 34 类分析（scripts/run_all.py）
make test          # pytest tests/ -q
make audit         # 表格无容器 + JS 语法 + SVG 负宽度检查
make lint          # ruff + eslint
make links         # 全仓库链接校验（lint_links.py --dir .）
make ci            # lint + test + audit + links + data-validate + docs-index（本地预跑 CI）
make release       # 发布前体检（release.py）
make screenshots   # Playwright 双视口截图
```

### 5.2 交付门禁（提交前必跑）

```bash
python scripts/verify_delivery.py        # 全部门禁，核心全绿才可提交
python scripts/generate_csp.py --check   # CSP 0 漂移
python scripts/check_js_syntax.py --all  # 全站内联脚本语法（与 make ci / CI 同轨）
python scripts/check_structure.py        # 全站 CSS 结构平衡
python scripts/lint_links.py --dir .     # 全仓链接校验（与 make links 同轨）
```

### 5.3 站点本地浏览

```bash
# 纯静态，直接双击 site/index.html（file:// 直开，无需任何软件）
# 或用本地 http 服务（部分页面的 fetch 实时数据源需 http 模式）：
python -m http.server 8000   # 然后访问 http://127.0.0.1:8000/site/
```

### 5.4 Web Agent 运行

```bash
cd xiyouji-agent-web
npm install
npm run dev          # 同时拉后端 :3000 + 前端 :5173
# 需先复制 .env.example → .env 并填入 CODEBUDDY_API_KEY
```

### 5.5 测试

```bash
pytest tests -q                        # pytest 全量（含 MCP 工具测试）
cd scripts && npm install && npm run test:e2e   # 三层 E2E
```

---

## 6. 必须遵守的铁律

1. **六文档同步（W393 降级）**：每完成一个 W 批次，核心 2（CHANGELOG + 交接文档）必须写，辅助 4（README/STRUCTURE/项目说明/file-index）里程碑时跑 bump_version.py 补齐。
2. **E1 铁律（声明 ≠ 落地）**：每个文件修改后 Grep spot-check 验证落地，禁止假收敛。
3. **改内联脚本 → 重跑 generate_csp.py**（CSP 哈希失配会整脚本被拒）。
4. **批量改 CSS/JS → 验证括号/引号平衡**（check_structure.py）。
5. **file:// 铁律**：新页面必须内嵌 EMBEDDED 回退、零外域依赖（D3/Three 本地引用 `site/static/js/`，禁 d3js.org/cdnjs 外域 CDN）。
6. **新页落地**：可视化页进 site/data/、索引进 site/ 根 + tag-cloud + sitemap；禁往 site/chapters|characters|themes（空遗留目录）。
7. **先取证再假设**：诊断前端显示问题，先拿截图 + Console + 复现条件，再下根因结论（见 docs/10-方法论沉淀/前端显示问题诊断SOP.md）。
8. **禁止擅改**：CHANGELOG 历史段、归档 3 份、.env、SECURITY-AUDIT 档、verify_delivery.py、bump_version.py 等门禁脚本（详见文档规范 §11.2）。
9. **禁重跑 w286_merge_yuanwen_shendu.py**（A1 深度解读 SD 编号会再次错位）。
10. **批量重写脚本改完**：`git diff --name-only` 对比改动范围，非必要改动 `git restore` 回退。
11. **双协议**：代码 MIT + 文本 CC BY-NC 4.0（非商用），拆分 LICENSE / LICENSE-CONTENT.md。
12. **动效契约（W463 固化）**：新增/修改可视化页动效必须遵守 DESIGN.md §5——时长 ≤600ms（白名单仅 hero 600 / count-up 900）、reduced-motion JS 守卫（D3 transition 不受 CSS media 控制，须 `MOYUN_RM` 调用点级或 prototype 级短路）、tooltip 统一 `.chart-tooltip` + `.classed('visible')`；批量改动后重跑 generate_csp.py + duration≤600 扫描。
13. **内容可信度轨（W505-W508 固化）**：docs/01-06 新内容必须走元信息块 v2（第 18 门禁）+ 术语一致性（第 19 门禁）+ 引文硬验证（第 20 门禁）；引文须经 `_cite_probe.py` 提取候选（§4.3），管线产出须过 `_check_pipeline_consistency.py`；归档/删除脚本须同步查 tests/ 引用（W506 教训，见 day-review skill 步骤 4-6）。

---

## 7. 快速上手路径（新 Agent 接手）

1. 读 `交接文档.md`「零、当前阻塞」+「一、当前进度」→ 当前 HEAD（vX.Y.Z W###）、下一 W 编号、遗留待办。
2. 读 `README.md` → 项目全貌、内容规模、在线站点、双协议。
3. 读 `docs/00-导读/文档规范.md` §11 → 文件管控清单（必同步/禁擅改/接手速查 6 步/同步核对 10 项）。
4. 读 `交接文档.md`「三、方法论沉淀」→ 可复利经验。
5. 深挖结构看 `STRUCTURE.md`、`scripts/output/file-index.md`（反向索引）、`CHANGELOG.md`（正向时间线）。
6. 动手前跑 `python scripts/verify_delivery.py` 确认基线全绿。

---

## 8. 权威文档（冲突时以此为准）

- 进度中枢 / 规则：`交接文档.md`、`新Agent启动Prompt.md`（速用精简版）
- 文档规范：`docs/00-导读/文档规范.md`（尤其 §11 文件管控）
- 设计规范：`DESIGN.md`（§1-4 视觉 / **§5 动效契约**）
- 目录结构：`STRUCTURE.md`
- 变更日志：`CHANGELOG.md`（正向）+ `scripts/output/file-index.md`（反向）
- 诊断 SOP：`docs/10-方法论沉淀/前端显示问题诊断SOP.md`

---

*本文件由 AGENTS 通读项目后生成于 2026-08-16（v2.3.73 W458），2026-08-17（v2.3.78 W463）墨韵系列复盘增补动效契约（§1/§6-12/§8）与 W 批收尾坑（§4.3），2026-08-18（v2.3.80 W477）增补维护契约与文档治理段，2026-08-19（v2.3.83 W484）skills 治理：§4.5 补录 4 个会话流程 skill（共 14 个），2026-08-19（v2.3.84 W485）收录视觉/方案专项 3 skill（visual-batch / plan-authoring / plan-review，共 17 个），2026-08-19（v2.3.85 W486）四会话流程 skill 协议同步（verdict 7 词 / R0 4 件套 / 警报增强 / P2 残留 / work-log 路径 / 整合版协调），2026-08-19（v2.3.86 W487）四会话流程 skill 二轮同步（DRL 未安装降级声明 + experience-capture 格式规范 + 触发词扩充），2026-08-22（v2.3.93 W494）Phase E 收官同步：§4.2 补录 W493 转正三门禁（token 覆盖率 / 动效禁止清单 / a11y 对比度，现 14 条），2026-08-22（v2.3.94 W495）P0 热修复同步：§4.2 补录 INLINED CSS 完整性门禁（现 15 条），2026-08-22（v2.3.95 W496）优化收尾同步：§4.3 补录验收数字当批现测铁律（acceptance_snapshot.py），2026-08-24（v2.3.96 W497）skills 治理同步：§4.5 补录 day-review（流程类 4 个·共 18 个）+ 版本脚注，2026-08-24（v2.3.97 W498）防漂移门禁：§4.2 补录第 16 门禁 check_skills_index（Skills 索引一致性）+ §4.5 注明配套 sync_skills.py 同步工具，2026-08-24（v2.3.98 W499）GitHub 协作模板 + 创意方法论沉淀批次：无 § 结构性变更（.github issue/PR 模板 + 创意方法论 2 篇 + character-content 创意方法引用 + 方法论 README 索引修复 + file-index 结构注记），仅版本脚注同步，2026-08-24（v2.3.99 W500）索引健康门禁：§4.2 补录第 17 门禁 check_index_health（file-index 段完整性/方法论 README 覆盖/编号上限·豁免 W449-W463 损坏区），2026-08-24（v2.3.100 W501）内容可信度轨启动：§4.2 补录第 18 门禁 check_frontmatter（元信息块 v2·新文件血缘+核验状态 4 字段·基线豁免 611 篇·口径文档规范 §4.6），2026-08-24（v2.3.101 W502）术语一致性门禁：§4.2 补录第 19 门禁 check_glossary（术语库类型化·称谓组规范词锚定·基线豁免 303 篇 383 条·口径文档规范 §4.7），2026-08-24（v2.3.102 W503）原著引文硬验证：§4.2 补录第 20 门禁 check_citations（引文行对 text-search.json 精确命中·防 AI 幻觉引文·口径文档规范 §4.8），2026-08-25（v2.3.103 W504）存量核验状态基线：无 § 结构性变更（611 篇核验状态全覆盖 + 学术轨 105 篇 A+ 引文核验·content-trust-report 产出），仅版本脚注同步，2026-08-25（v2.3.104 W505）创意流程闭环落地：无 § 结构性变更（character-content 新增创意三明治管线章节 + 4 篇试点方向二深化），仅版本脚注同步，2026-08-25（v2.3.105 W506）处置遗留：无 § 结构性变更（删除失锚测试 test_fix_svg_negative_widths.py），仅版本脚注同步，2026-08-25（v2.3.106 W507）复盘沉淀落地：无 § 结构性变更（引文探针 _cite_probe.py 永久化 + day-review 归档查测试规则），仅版本脚注同步，2026-08-25（v2.3.107 W508）复盘剩余项收口：无 § 结构性变更（creative-methods 方法四去重指向 SKILL 管线章节 + _check_pipeline_consistency.py 管线一致性轻量校验），仅版本脚注同步，2026-08-25（v2.3.108 W509）经验上移共享：§4.3 工具链新增 3 条强制规则（Set-Content BOM 禁 / 同文件 Edit 串行 / 引文探针+canonical 预查）+ §6 铁律新增第 13 条内容可信度轨，2026-08-25（v2.3.109 W510）治理文档修复：无 § 结构性变更（文档规范 §8 门禁数 17→20 + §11.4 核心口径统一），仅版本脚注同步，2026-08-25（v2.3.110 W511）治理文档健康指标归档：无 § 结构性变更（CHANGELOG/file-index/交接文档概要 三档瘦身·W417-W464 段归档），仅版本脚注同步，2026-08-25（v2.3.111 W512）CI 安全批次：无 § 结构性变更（security_scan pip-audit 超时 120→300s·DEP-001 误报归零），仅版本脚注同步，2026-08-25（v2.3.112 W513）归档二级归档：无 § 结构性变更（CHANGELOG-ARCHIVE W001-W399 下移 docs/archive/tier2·归档三件套 >1MB 触发规则入文档规范 §5/§8），仅版本脚注同步，2026-08-25（v2.3.113 W514）治理文档口径修复：§1 规模数字校正（核心内容 611→615·A3 人物 211→215·「约 86 个可视化页」注明＝site/data 87 个 HTML 减 _shell.html）+ §4.2 第 3 条改为动态口径（磁盘==README 声明）+ §4.2 补录第 21 门禁 check_dynamic_links（动态链接）与第 22 门禁 check_governance_docs（治理契约），2026-08-25（v2.3.114 W515）渲染抽查常驻化与门禁扩展：§4.2 第 16 门禁补录正文引用资产存在性子检查（堵 _shot_check.js 式失效指针），无其他 § 结构性变更，仅版本脚注同步，2026-08-25（v2.3.115 W516）经验上移机制固化：§4.3 补录 Windows heredoc 禁（Write 临时文件 + -F 参数）+ 跨 session 先确认远端 / workflow CI 类经验（E41/E36-42·详见交接文档「三」W516 段）；mem-wrap-up Step 5 固化上移检查子步骤（毕业路径 + 上移映射表 + 强制登记交接文档「三」），2026-08-25（v2.3.116 W517）共享机制载体铁律：§4.3 补录「共享机制必须写入仓库内文件（git tracked）」——禁只写全局路径（其他 Agent 读仓库版 + sync_skills 仓库→全局单向·W516 载体错误教训固化），2026-08-25（v2.3.117 W518）期望版本动态化 + 尾页脚新鲜度门禁：§4.2 第 1 条补录动态版本源（verify_delivery 期望版本动态取自 CHANGELOG 现役版段·dukou-engine.html 页脚降级为新鲜度 WARN）+ 第 22 门禁扩为七项检查（新增「最后更新」新鲜度：头尾两处链首现役校验·finditer 全量），2026-08-25（v2.3.118 W519）skills 全目录审查与 SKILL.md 内容优化：§3 目录树注释 skill 数 17→19（README 与 §4.5 本就正确·树注释漏更修正），无其他 § 结构性变更，2026-08-25（v2.3.119 W520）递增数字禁字面量 + skills 内文数字比对维度：无 § 结构性变更（plan-authoring 去模糊化标准⑤ + day-review 步骤 4 第 8 条 + drift-audit 步骤 5 第 4 条固化·E46 登记交接文档「三」，E45 已被 W460-W463 三坑占用顺延），仅版本脚注同步，2026-08-25（v2.3.120 W521）存量裸字面量清剿 + W463 三坑补登记：无 § 结构性变更（DRL F2 15 文件三分类裁决——现状声明改引用式/口径加注/门禁模板豁免·交接文档「三」补 W463 段 E45 编号空间完整化），仅版本脚注同步，2026-08-25（v2.3.121 W522）CI 红灯修复：无 § 结构性变更（scripts/ ruff 16 错误清零——inject_goatcounter.py:68 字符串未闭合根因修复 + 12 lint 机械清理 7 文件·行为零变更），仅版本脚注同步，2026-08-25（v2.3.122 W523）截图审查恒全量根因修复：§4.3 工具链新增「workflow/脚本用 git diff 输出做路径分类必须加 -c core.quotePath=false」条目（中文路径八进制转义致 case 匹配失效·曾静默架空 W421 截图提速机制），2026-08-25（v2.3.123 W524）bump 追加污染坑位补记：§4.3 bump_version.py 条目修订——已知坑②扩为 --desc/--note 双触发（W523 实测 --note 同样追加污染·非仅 --desc）+ 坑①补充 file-index 空壳段现象，2026-08-26（v2.3.124 W525）漂移审查修复：无 § 结构性变更（drift-audit 处置 file-index W504 段缺失补建 + W522-W524 尾部追加重排归位·workflows README 同步至 W524·文档规范 §8 门禁表对齐 AGENTS §4.2 22 条），2026-08-26（v2.3.125 W526）索引健康门禁盲区封堵：§4.2 第 17 门禁扩展（段倒序断言 + 段缺失检测）+ 新增第 23 门禁 check 旁文档同步（workflows/README.md 头部版本行 + 里程碑行上限 == 现役 W），2026-08-26（v2.3.126 W527）drift-audit 技能补漏维度：无 § 结构性变更（SKILL.md v1.2.0 步骤 3 段缺失双向核对 + 豁免区外新增乱序 P1 判级——W525 实证对齐·reference 命令/案例同步·sync 双轨一致），2026-08-26（v2.3.127 W528）存量漂移点统一修复：§4.3 bump_version 条目扩坑③（三简单页脚替换式盲区·滞后即永久脱节——W528 实证停 W453 脱节 74 批）+ 交接文档「三」W528 段登记；六文档 9 处静态描述对齐现役口径（skills 17→19·A3 211→215·学术索引 50→55·site/data 46→86·D3 本地化·README 索引去版本号）+ index/cross-time-danmaku/tag-cloud 三页脚补现役 W528，2026-08-26（v2.3.128 W529）R6 拦截落地 + W464 方案 v7 回填：§4.3 bump_version 坑③修复首次运行时验证（三页脚链首==解析版本被正常替换）；无其他 § 结构性变更（goatcounter beacon 纵深防御拦截 5 点 + 方案 12 处表述具体化），2026-08-26（v2.3.129 W530）决策闸门工程落地：无 § 结构性变更（judge_gate.py 新建 + 读者数据复盘模板 + 方法论索引登记），仅版本脚注同步，2026-08-29（v2.3.130 W531）skills 三真源降级保护：§4.2 第 16 门禁配套工具条目修订（sync_skills 增 judge_direction 方向判定 + --sync 降级拦截 + --take-global 反向回写 + LF 归一化比对 + --self-test 4 负样本——W531 实证旧 --sync 会把 QwenWork 侧演进的 4 个通用技能静默降级），2026-08-29（v2.3.131 W532）交接文档滚动链治理：无 § 结构性变更（第 7 行「最后更新」9 批堆叠裁回维护契约②上限 3 批·删 6 批前逐批断言 CHANGELOG 有段·第 22 门禁只校验链首不校验长度），2026-08-29（v2.3.132 W533）skills 归属策略显式化：§4.5 四通用会话流程 skill 标注 master=全局安装版/仓库副本为受控镜像（MIRROR_SKILLS 禁 --sync/--force），§4.2 第 16 门禁条目补记该策略与 --self-test 5 负样本，2026-08-30（v2.3.133 W534）治理文档递增数字字面量修复：无 § 结构性变更（交接文档「九、使用说明」第 2 条「接续 W 编号」写死 W 号改引用式表述 + 六份治理文档全量复扫 0 残留），仅版本脚注同步；2026-08-30（v2.3.134 W535）决策闸门取数自动化：§4.3 bump_version 条目新增坑⑨（全文模式替换会污染交接文档历史段·检测用逐行相似度 0.80≤r<1.0 再过滤 LF/CRLF 噪声）+ 坑⑤适用面由「git commit -F」扩至 Windows 原生程序一切路径实参（version-bump v1.6.0 收录）·无其他 § 结构性变更；2026-09-02（v2.3.135 W536）依赖积压治理：无 § 结构性变更（dependabot ignore semver-major + eslint.config.mjs 新建·迁移评估出档；收尾补 Mimosa 闸门 77 高危清零——全仓脚本写路径守卫/去动态执行/请求边界加固/server 工作目录钳制），仅版本脚注同步；2026-09-05（v2.3.136 W537）全仓对抗性审查修复：§4.3 新增 W537 三新规·§4.3 sync 指引补 MIRROR_SKILLS 例外交叉引用·§4.4 权限默认值校正·§5.2 命令对齐 CI 同轨；2026-09-05（v2.3.137 W538）CI 红灯热修复：无 § 结构性变更（batch_screenshots 白名单形状回归修复 + agent-web 依赖 overrides），仅版本脚注同步；2026-09-05（v2.3.138 W539）遗留收尾：无 § 结构性变更（vite 5→6.4.3 清零 devDep 漏洞·dompurify 3.4.14·移除 dompurify override 根治 Dependabot 冲突），仅版本脚注同步；2026-09-05（v2.3.139 W540）迁移评估基线更新：无 § 结构性变更（docs/10 评估文档 vite 基线 6.4.3 注记），仅版本脚注同步；2026-09-05（v2.3.140 W541）复盘报告归档：无 § 结构性变更（docs/10 新增工作复盘与优化分析报告·方法论 README 索引第 21 条），仅版本脚注同步；2026-09-05（v2.3.141 W542）交付效率工具化：新增 batch_cascade.py 级联脚本常驻化（§4.3 三新规④配套工具）·§4.3 三新规增补④（JS/工具脚本推送前真实参数冒烟）·本批级联由本脚本 dogfood 完成，仅版本脚注同步；2026-09-05（v2.3.142 W543）agent-web 迁移阶段 1：Express 5 / uuid 14 / dotenv 17 / @types/node 26 / better-sqlite3 13 落地（运行冒烟通过·评估未验证项出清），仅版本脚注同步；2026-09-05（v2.3.143 W544）agent-web 迁移阶段 2：TypeScript 7 + @types/react 19 类型基线归零（TS2882/TS2322 修复），仅版本脚注同步；2026-09-05（v2.3.144 W545）热修复补记：batch_cascade.py _root 未定义一行修复（ruff F821），仅版本脚注同步；2026-09-05（v2.3.145 W546）迁移阶段 3：Vite 8.2（rolldown）+ plugin-react 6·engines 收紧，仅版本脚注同步；2026-09-05（v2.3.146 W547）迁移阶段 4：Tailwind v4 范式迁移（@tailwindcss/postcss + @config 兼容 + dark 变体），仅版本脚注同步；2026-09-05（v2.3.148 W548）迁移阶段 5 + 闭环：React 19 + TDesign 1.18 落地（人工走查待办）·dependabot ignore 解除，仅版本脚注同步；2026-09-05（v2.3.149 W549）复盘报告后记：WBS 5/5 对账与迁移闭环补记，仅版本脚注同步；2026-09-05（v2.3.150 W550）全站截图审查与修复闭环：§4.3 补录「扫描关键字须穷尽同义变体」规则（圆环图变体漏网教训）·审查报告与全部验证产物存 tmpe/（未入库）·generate_csp.py W536 写路径守卫根目录误算修复；2026-09-05（v2.3.151 W551）图表门禁常驻化：§4.2 补录第 24 门禁 check_chart_data（sankey 引用/饼图错配）+ 动态五类 check_screenshot_gates.js 挂 screenshot-review workflow·基线 234 页全绿；2026-09-06（v2.3.152 W552）三项可选改进批次计划入档 docs/superpowers/plans（A 低危细节/B 全站复审/C 数据审计·均含机判验收）；tmpe 产物删除·W550 审查报告精简归档 docs/archive/w550-shot-review；2026-09-06（v2.3.153 W553）低危视觉细节批次：§4.3 补录 SVG 标签量测四坑（getBBox 局部坐标系跨组比较幻影重叠 / tspan 显式 x 在 x 定位 text 中是 g 原点绝对坐标 / selection.text() 抹子元素 / canvas 量测比真实渲染窄 9% 须 svg 探针 getComputedTextLength）·data-audit-skip 豁免机制；2026-09-06（v2.3.154 W554）方案B阶段一+C数据审计：§4.3 补录 fullPage 截图不触发 IntersectionObserver（含 reveal-in 页面截图前必须滚动穿透·G4 教训二犯固化）·EMBEDDED_DATA 回退铁律新增部署态 fetch 外部路径反例（en/81-hardships）；2026-09-06（v2.3.155 W555）工作复盘与优化分析报告入库 docs/10-方法论沉淀（续 2026-09-05 前序）：无 § 结构性变更（行动项待执行时再行入档），仅版本脚注同步；2026-09-06（v2.3.156 W556）审计修复批次：§4.2 补录第 25 门禁 check_content_consistency（--gate + 基线冻结·经用户确认）·F1-F8 修复落地；2026-09-06（v2.3.157 W557）全站复审完成：§4.3 补录 contentavoid 全局根治（getBBox→getBoundingClientRect·页面级豁免≠类绝迹）与「标记与 CSS 演化脱节查基类」两条经验；2026-09-07（v2.3.158 W558）五类残留缺陷修复：§4.3 补录「CSS 缺分号 = 整条声明静默丢弃」条目（全站 130 页 1006 处根治·与基类丢失同属属性静默丢失族）；2026-09-08（v2.3.159 W559）工作复盘与优化分析报告入库 docs/10-方法论沉淀（续前两篇）：无 § 结构性变更（行动项随下批执行），仅版本脚注同步；2026-09-08（v2.3.160 W560）工具链收尾批次：§4.3 W559 复盘 WBS 三项落地——batch_cascade 写后自检自愈/CSP 守卫接入探针脚本/L2 运行时对账 39/39（--dataset-runtime）；2026-09-08（v2.3.161 W561）残余几何越界收敛：>60px 38→9 处（chart-minions 同款 width 陷阱补齐——同文件同模式修复须全文件排查）；S1 级联验收连续 2 批零人工修复达成（P05 关闭）；2026-09-08（v2.3.162 W562）skills/ 目录退役：AGENTS §4.5 移除·第 16 门禁与 sync_skills 同步退役（编号保留·经用户确认）；2026-09-10（v2.3.163 W563）前端关键路径与部署正确性：D3 head 阻塞 154→0（sankey 链式同移·defer 不安全实证）·inline_css 深度 url 重写+W536 守卫根目录修复（225 子页字体首次生效）·越界 fetch 全树清零（F8 正则漏 baseUrl 形态教训·47 副本）·text_loader 第*.md bug 根治（chapter_stats/character_appearance 首产真实数据）·SW 摘死字体+SWR+SHA 版本戳·门禁 9 副本对账扩展；2026-09-10（v2.3.164 W564）字体字节治理：三字体按站内字符集子集化 -309KB（serif 源 1499 字形与站内字符集完全一致零可减·sans 4455 字形为地板——按 2181 字预估的 300KB 门槛失准如实修订）·226 页主字体 preload 每字体恰 1 次请求·覆盖守卫常驻（零回归语义=子集⊇字符集∩源cmap+serif fvar 断言）·SW SHELL 联动 subset；2026-09-12（v2.3.165 W565）EMBEDDED单源化与预算收紧前置：relationships 中英双源消除（-465KB·其余 15 页登记保留 fetch）·部署态冒烟抓出 2 个真崩溃根治（relationships baseUrl 残留引用·character-appearance 契约失配生产环境自 W563 起崩溃——file:// 冒烟≠部署态教训）·perf.yml artifact path 根因修复（lhci 无点 lighthouseci/ 目录）·L2 首跑 2 存量漂移登记；2026-09-13（v2.3.166 W566）遗留三项裁决与收口：L2 存量漂移清零（dataset 向站侧对齐·six-senses sankey 40→56）·81-hardships 码标签渲染缺陷根治（页内「数据用码+渲染用标签」双轨静默降级教训）·text-search CLS 0.315→0.075（late 注入容器量高占位）·link 页 preload 补全 6 页；2026-09-13（v2.3.167 W567）工作复盘报告入库：W563-W566 前端治理会话复盘（经验复用 9 项·问题 19 例闭环·两项根因 5Why——静默降级类缺陷在缺陷类清单之外·EMBEDDED 回退使 file:// 与部署态成两条独立验收路径·judge 0 委托全机判对照·LCP 债关闭）；2026-09-13（v2.3.168 W568）聊天页视觉走查与错误链路根治：Express5/Node20+ req close 语义陷阱（请求体读完即触发·SSE 事件整段丢失·改挂 res close）+ 前端 error 分支缺失与末 chunk 丢弃双修（思考中永久挂起→⚠️ 气泡）·AGENTS §4.3 落载体收尾七步/取证三陷阱/部署态冒烟五条（W548 遗留走查项关闭）；2026-09-16（v2.3.169 W569）渲染状态全站审计与图表暗色适配 v2：审计器常驻（232页×3态·夜间模式盲区补上·暗色自动应用 230 页）·tokens 暗色块文字反白（暗色缺陷 1590→302 -81%·132→42 页）·4 个 pageerror 页契约错位根治（W563 数据打通暴露家族·滚动穿透复验 0）；2026-09-16（v2.3.170 W570）审计矩阵补全与图表页容错封装：审计器补 S4 移动浅色（2×2 完整·S4 与 S1 基线一致证实视口非低对比变量）·4 页 44 处渲染调用 safe() 容错封装（单区块崩溃不再杀整页·pageerror 清零）·⚠️ 错误气泡定格留证·canvas 暗色扩样 5/5 可读；2026-09-16（v2.3.171 W571）复审修正：W569 一刀切反白误伤 168 光晕页（墨字白晕本双主题可读）→ tokens 规则 :not(:has(#audit-halo)) 豁免；en/ne 钳位补齐；审计器光晕感知——暗色真实基线 318 处/44 页（原 1590 含光晕假阳性）；2026-09-18（v2.3.172 W572）站点可达性与信任信号：无 § 结构性变更（D-1 越界锚链接 264→0 改写 GitHub blob/tree·D-2 自定义 404 页·D-3 全站页脚反馈入口 235/235·D-4 首页页脚收敛保留 bump 正则兼容形态），仅版本脚注同步；2026-09-18（v2.3.173 W573）站内检索服务化与RUM改道：无 § 结构性变更（E-1 全站目录索引 772docs+233pages 内嵌 search 两页离线可搜·renderOffline 打分重写·开发者文案清除·E-2 rum.js 协议闸门+GoatCounter __rum__ 单事件·file:// 零网络请求），仅版本脚注同步；2026-09-18（v2.3.174 W574）可视化页触屏tooltip试点：无 § 结构性变更（方案H 试点——language-style-radar touchstart 委托 25 行复用既有 show/hide·e2e 6/6·教训三条：CSP 先 regen 再 e2e/星形多边形顶点→重心取点/命中测试需视口内坐标），仅版本脚注同步；2026-09-18（v2.3.175 W575）可视化页触屏tooltip全量推开：§4.3 补录「d3.select 静态快照空选区族（选区先于元素执行/元素缺失/门面对象误调 .style·桌面鼠标同样静默受害·select 行 vs 元素行静态扫描可穷尽）」与「触屏 tooltip e2e 三坑（transition 下用内容同步信号+快照键勿含显隐 class / Chrome touchend 后 hover resync 补发 mouseout——触屏断言先于鼠标动作 / 泛选择器误伤+顶层容器过滤+常显面板≡鼠标对照）」两条；61 页同构注入·e2e 61/61·同族缺陷 4 页根治；2026-09-18（v2.3.176 W576）bump_version.py 守卫误算修复：§4.3 bump_version 条目补记一笔（W536 写路径守卫根目录误算经用户批准修复为项目根·与 W550 generate_csp 同款·工具恢复可用·范围替换面为级联外的存量漂移收敛手段），仅版本脚注同步；2026-09-18（v2.3.177 W577）工作复盘报告入库：W569-W571 渲染状态治理会话复盘（经验复用 7 项·问题 11 例闭环·两项 P1 根因 5Why——批量规则×设计惯例冲突缺前置排查·源-副本一致性无门禁·审计流程并行化方案·方法论 README 索引第 25 条）；2026-09-18（v2.3.178 W578）复盘报告 WBS 落地①：AGENTS §4.3 补齐 W571 两条教训条目（全局批量规则前置设计惯例三查/tokens 修改必跑 inline_css --force 分发——声明≠落地缺口补齐）·O2 暗色门禁常驻化挂载方案入档；2026-09-18（v2.3.179 W579）暗色态门禁常驻化挂载：无 § 结构性变更（O2 落地——screenshot-review workflow 增 dark-state-gate 独立 job·S2×163 图表页·基线 318/44 对账 W571·只增即 FAIL·hOverflow 偏离声明·双跑零差异+实弹验证），仅版本脚注同步；2026-09-18（v2.3.180 W580）CI 红灯热修复：无 § 结构性变更（dark-state-gate Checkout 补 fetch-depth: 0——浅克隆下 git diff $BEFORE $SHA 报 128·首跑实证），仅版本脚注同步；2026-09-19（v2.3.181 W581）触屏tooltip EN镜像推开：§4.3 触屏 tooltip e2e 三坑条目补记一笔（d3.drag 的 touchstarted stopImmediatePropagation 会拦截 document 冒泡层委托——注入升级 capture 阶段免疫；TouchEvent 同 tick 取点派发对抗力导向漂移；tap 式断言的 PASS 可能由 compat 事件代劳·须以事件流取证甄别），仅版本脚注同步；2026-09-19（v2.3.182 W582）暗色存量修复阶段一：无 § 结构性变更（tokens.css 暗色段 24 种离散 hex 填充映射——属性精确匹配·保色相调亮·浅色零改动；lowContrast 16→0·invisible 302→259·残余连续色阶页登记阶段二·基线刷新），仅版本脚注同步；2026-09-19（v2.3.183 W583）O3 契约对账门禁评估落地：无 § 结构性变更（运行时产量冒烟独立 CI step 挂 dark-state-gate job·file:// EMBEDDED 路径与暗色门禁 http 路径互补·P04 类契约错位拦截·字段级静态对账否决入档 docs/superpowers/plans/2026-09-19-o3-contract-gate-evaluation.md），仅版本脚注同步；2026-09-19（v2.3.184 W584）O1 审计器并行化：无 § 结构性变更（_audit_render_states.js state 内 3 并发 worker 共享 context·--conc 参数化·S2×163 实测 4.5×·与基线零差异），仅版本脚注同步；2026-09-19（v2.3.185 W585）注入头行mouseout补bubbles：无 § 结构性变更（升级②漏网的头行 124 页补 bubbles——非冒派发到命中子元素即止 g 包装层 hideTip 不触发·geo 实证；断言等待 300→650ms 对齐动效上限 400ms hide transition·B/D 升回阻断·终验 EN 59/62+ZH 58/61 余 6 页页面级残留登记），仅版本脚注同步；2026-09-19（v2.3.186 W586）注入祖先链 mouseleave+e2e 定型：无 § 结构性变更（hide 分支沿 prev→documentElement 祖先链派发 mouseleave——mouseleave 型 hide 页需祖先链命中·e2e 等待对齐动效上限/refBlank 触控垫在位拍摄/持久型内容匹配判定三处定型·B/D 全阻断 EN 62/62+ZH 61/61 warn=0），仅版本脚注同步；2026-09-20（v2.3.187 W587）暗色存量修复阶段二：无 § 结构性变更（运行时填充亮度提升器 163 页——dark 门控·亮度 <0.16 混合至 ≥0.22 保色相·fill=none/渐变/文字不碰·多时点采样·fail-open；invisible 259→0·缺陷页 37→0·基线刷新至零缺陷·暗色治理全链闭环），仅版本脚注同步；2026-09-20（v2.3.188 W588）暗色提升器 v2：无 § 结构性变更（定时采样→MutationObserver+防抖+去标记化幂等——CI 慢环境渲染时序竞态 31 项根治·本地复验 invisible 0·门禁绿），仅版本脚注同步；2026-09-20（v2.3.189 W589）暗色审计读数语义修正：无 § 结构性变更（invisible 判定改读 computed+透明 alpha<0.1 豁免+var() 引用豁免——原 getAttribute 优先读页面意图串致 CSS 覆盖后仍计旧暗值·两代映射合并重建·JS 提升器撤除·残余 48/7 页数据驱动角色色登记设计批），仅版本脚注同步；2026-09-21（v2.3.190 W590）需求侧优化主计划入库：无 § 结构性变更（13 工作包方案入档 docs/superpowers/plans·Mode B 自审 8 项修正回填·WP-A 取数待用户配 token），仅版本脚注同步；2026-09-21（v2.3.191 W591）SEO 五项硬伤清零：无 § 结构性变更（og/canonical/JSON-LD/hreflang/sitemap 全达标+三件常驻脚本·check_js_syntax 经批准跳过数据块·check_seo_head 待裁决挂第 26 门禁），仅版本脚注同步；2026-09-21（v2.3.192 W592）AI 入口收敛：无 § 结构性变更（首页 CTA 改道站内搜索·品牌去重 15 处·agent-web 定位声明·用户裁决暂不部署后端），仅版本脚注同步；2026-09-21（v2.3.193 W593）docs 站内阅读器试点：无 § 结构性变更（site/reader 101 页·A1 搜索索引站内化·导航机判 100/100·冒烟 3/3·一致性基线 +5 冻结），仅版本脚注同步；2026-09-21（v2.3.194 W594）搜索质量与搜索词埋点：无 § 结构性变更（kw 加权+4·双索引分离·黄金查询 30/30·上报双通道·rum e2e 适配 11/11），仅版本脚注同步；2026-09-21（v2.3.195 W595）CI 红灯热修复：无 § 结构性变更（W593 漏 add 的 sitemap.xml 补提交 330 条·教训登记方案 WP-C 第 5 条），仅版本脚注同步；2026-09-21（v2.3.196 W596）Agent 运行卫生：无 § 结构性变更（§4.4 PROJECT_CWD 表述更新·脏路径活面清零 14 文件·PROJECT_CWD 自动解析+fail-fast·Docker 三件套入库待补验），仅版本脚注同步；2026-09-21（v2.3.197 W597）一次性脚本治理：无 § 结构性变更（_attic 收档 25+未跟踪 42 清零+需求侧批次配额规则·90 天规则执行期修订为 ≥45 天并登记），仅版本脚注同步；2026-09-21（v2.3.198 W598）Agent 黄金评估集：无 § 结构性变更（evals 三件套·CI validate 挂载·50/50+4/4 机检·LLM 基线待凭证），仅版本脚注同步；2026-09-21（v2.3.199 W599）Agent 引用校验与拒答边界：无 § 结构性变更（citationGuard+SSE 回写+双提示词拒答与引用条款·单测 5/5），仅版本脚注同步；2026-09-21（v2.3.200 W600）Agent 反馈闭环：无 § 结构性变更（操作条四按钮+feedback 幂等表+两端点+耗时展示·集成测试 5/5·UI 点按 e2e 待凭证），仅版本脚注同步；2026-09-21（v2.3.201 W601）EN 站治理：无 § 结构性变更（页脚双向互链 89/89·泄露清理·subset 量化·互链落位页脚为登记偏离），仅版本脚注同步；2026-09-21（v2.3.202 W602）A1 信任字段补全：无 § 结构性变更（100 篇三字段回填·git 日期机器取值·核验状态空真防护登记·511 篇否决维持），仅版本脚注同步；2026-09-21（v2.3.203 W603）CI 热修复：无 § 结构性变更（LHCI 新审计暴露的 brand/页脚链接 a11y 全站修复·本地复测 0.92→0.96·根因为 lhci 浮动版本工具链漂移），仅版本脚注同步；2026-09-22（v2.3.204 W604）S4 学术投稿 B 轨入库：无 § 结构性变更（《装饰》投稿版 v2.1 摘要收敛+字数改当批实测·匿名稿重同步·9 图与配套文档 19 文件），仅版本脚注同步；2026-09-22（v2.3.205 W605）S4 学术核查修正：无 § 结构性变更（幻觉文献全仓清除 7 处·论文事实校准 4 项·论证加固 4 项——详见 CHANGELOG），仅版本脚注同步；2026-09-22（v2.3.206 W606）S4 论文语域专业化：无 § 结构性变更（去博客腔三稿 40 处·文献激活 [1]-[5]·图 1/2 硬伤根治与图 6 统一·重绘脚本入库），仅版本脚注同步；2026-09-22（v2.3.207 W607）S4 论文 Word 化与文献库：无 § 结构性变更（投稿 docx 双稿生成·Word COM 渲染验收 13/13·Zotero 文献 JSON 21 条·图注引用补全），仅版本脚注同步；2026-09-23（v2.3.208 W608）匿名稿图 5 双盲脱敏：无 § 结构性变更（品牌栏裁除 175px·衍生图仅匿名稿引用·docx 重生成 Word 渲染复验 8/8 pass），仅版本脚注同步；2026-09-23（v2.3.209 W609）外部审读采纳四项：无 § 结构性变更（图 6 改研究设计示意·bezier 实值·⑥⑧文献核验补全·表述中文化·词计回收区间），仅版本脚注同步；2026-09-23（v2.3.210 W610）第二份外部审读分级采纳：无 § 结构性变更（表 2/表 3 真实值补强·三小修·驳回虚构令牌名与错色值·字数口径修正为稿件口径），仅版本脚注同步；2026-09-23（v2.3.211 W611）第三份审读微采纳：无 § 结构性变更（表 2 题注写法+英文关键词斜体·P0 指控三度证伪），仅版本脚注同步；2026-09-23（v2.3.212 W612）用户研究整体删除：无 § 结构性变更（协议/图 6/相关段落全链清除·⑯→⑮ 重排·Zotero 20 条·论文定稿为纯设计系统型），仅版本脚注同步；2026-09-23（v2.3.213 W613）S4 三线推进：无 § 结构性变更（B 轨投稿辅助三草稿·A 轨强化六项达标·C 轨大纲立），仅版本脚注同步；2026-09-23（v2.3.214 W614）第四份外部审读分级采纳：无 § 结构性变更（B 轨文献体例微调+字数口径 Word 实测），仅版本脚注同步；2026-09-23（v2.3.215 W615）官网体例定稿：无 § 结构性变更（B 轨注释重排+AI 文末声明+字符口径瘦身+政策核查官网直采），仅版本脚注同步；2026-09-24（v2.3.216 W616）《装饰》投稿包独立文件夹迁移：无 § 结构性变更（S4 目录重组·B 轨包迁入 装饰投稿/ 子目录），仅版本脚注同步；2026-09-24（v2.3.217 W617）投稿版人文化改写：无 § 结构性变更（B 轨文风重写+字数精剪），仅版本脚注同步；2026-09-24（v2.3.218 W618）图片 AI 率处置：无 § 结构性变更（B 轨声明补图源+重绘底稿+脚本修复），仅版本脚注同步；2026-09-24（v2.3.219 W619）官方投稿须知全文终验：无 § 结构性变更（B 轨声明补流程句+作者信息占位+风险挂载），仅版本脚注同步；2026-09-24（v2.3.220 W620）装饰投稿包五图 SVG 重绘：无 § 结构性变更（B 轨图源升级·生成器入库·投稿包本地交付），仅版本脚注同步。如与上述权威文档冲突，以权威文档为准。*
