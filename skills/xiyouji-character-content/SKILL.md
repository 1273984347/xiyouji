...
- **深化专题**：必须有原著引文（标注回目）+ 理论框架 + 与既有深化专题的互链（`与 W152 天庭体系深化...形成体系`）。**W503 硬规则**：方向二深化/深化专题/学术轨新文档须含 ≥3 条 `> 原文引文（第N回）：“…”` 引文行，且 `python scripts/check_citations.py --file <本文件>` 命中率 100%（语法见文档规范 §4.8）。
