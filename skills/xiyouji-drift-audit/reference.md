...
```bash
# 段结构检查（门禁已自动化：段倒序断言 + 段缺失检测，W526 起；人工复核聚焦豁免区 W449-W463 边界 + 双向差集）
grep -n "^## W" scripts/output/file-index.md | head -50        # 段标题序列——人工核对倒序（最新在前）
grep -c "^| W" scripts/output/file-index.md                      # 登记行数
# 段缺失（双向）：CHANGELOG 现役版段 vs file-index 段差集（W504 实证：CHANGELOG 有段而反向索引无登记 = P1）
grep -E "^### v[0-9.]+（[^）]*）：W[0-9]{3}" CHANGELOG.md | grep -oE "W[0-9]{3}" | sort -u > /tmp/cl_w.txt
grep "^## W" scripts/output/file-index.md | grep -oE "W[0-9]{3}" | sort -u > /tmp/fi_w.txt
comm -23 /tmp/cl_w.txt /tmp/fi_w.txt   # CHANGELOG 有而 file-index 无 = 段缺失（P1）
comm -13 /tmp/cl_w.txt /tmp/fi_w.txt   # file-index 有而 CHANGELOG 无 = 冗余段（核对是否历史归档）
# 豁免区外顺序错位：新增错位（W522-W524 式尾部追加）= P1；既有错位（W464 式）= P3，重排需用户授权
```
...
- **现象**：段空壳/重复/乱序/段缺失/残留"当前版本"快照行。
- **案例**：W449-W463 历史损坏区（已豁免，维持现状不重排）；W499 后第 17 门禁只防豁免区外新增；W504 段缺失实证——CHANGELOG 有 v2.3.103 段而反向索引无 W504 登记段（双索引铁律违反，W525 审查发现、W526 转自动化检测）。
- **判据**：豁免区外空壳段/重复 W 号/最新段残留/段缺失 = FAIL（段缺失判 P1）；豁免区内 = 记录不判。
...
### 类型 I：file-index 豁免区外乱序（P1/P3 分级，2026-08-24 首跑发现并修复 W464 既有错位；W525 扩展分级）
- **现象**：豁免区（W449-W463）之外的历史段顺序错位——W464 段曾夹在 W484 与 W478 之间（倒序应为 ...W484 → W478 → W477 → W476 → W464...）；W522-W524 曾被追加到文件尾部（W525 审查发现）。
- **判级**：豁免区外**新增**错位（W522-W524 式尾部追加）= **P1**（W525 实证判级并修复——新增即破坏倒序登记语义）；**既有**错位（W464 式非新增历史遗留）= P3（不影响登记语义）。与类型 A 区分关键在"是否本次新增"。
- **门禁**：W526 起第 17 门禁新增段倒序断言（豁免区外严格递减），新增错位由门禁直接拦截；人工复核为兜底（豁免区边界 + 既有错位识别）。
...
### 类型 J：W525 实证——段缺失 + 新增乱序双漏网（P1，2026-08-25 审查发现 / W526 转自动化）
- **现象**：W525 审查实测第 17 门禁不查缺失段/不查顺序，两起漂移均漏网、全靠人工兜底——①W504 段缺失（CHANGELOG 有 v2.3.103 段而 file-index 无登记段）；②W522-W524 被追加到文件尾部（豁免区外新增乱序）。
- **处置**：两起均判 P1 并修复；W526 将段倒序断言 + 段缺失检测落入 check_index_health（第 17 门禁检查 6/7），此后新增同类漂移由 pre-commit 直接拦截，本技能人工复核降级为豁免区边界 + 双向差集兜底。
- **教训**：技能判级须与实测处置一致——本技能旧版（1.1.0）把豁免区外新增乱序记为 P3（仅记录），与 W525 实际判 P1 不符，1.2.0 已修正（SKILL 步骤 3 + 类型 I 判级同步）。
