#!/usr/bin/env python3
"""W478 E2 剩余 53 页迁移（一次性脚本，不入门禁/CI）

忠实复刻试点 3549327 的迁移范式（R-SHADOW/R-RADIUS/R-TRANS/R-FOCUS/裸色白名单/R-EXEMPT）。
仅处理页私有 <style> 块（跳过 INLINED tokens+system 块）；禁全站盲正则——页面清单由派生命令生成。

用法：
  python scripts/_w478_migrate.py --dry    # 仅输出拟改行供审查
  python scripts/_w478_migrate.py          # 实际应用
"""
import re
import subprocess
import sys
...
def derive_list():
    net, heat = set(), set()
    for p in DATA.glob("*.html"):
        if p.name == "_shell.html":
            continue
        t = p.read_text(encoding="utf-8", errors="ignore")
        if "forceSimulation" in t:
            net.add(p)
        if re.search(r"scaleBand|heatmap|histogram", t):
            heat.add(p)
    pages = sorted(net | heat)
    return [p for p in pages if p.name not in PILOTS]
