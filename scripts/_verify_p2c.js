...
const fs = require('fs');
const PAGES = ["jurisprudence.html", "81-hardships.html"];
...
(async () => {
  ...
  for (const pg of PAGES) {
    ...
    page.on('console', m => { if (m.type() === 'error') { const t = m.text(); if (!/Failed to fetch|ERR_FILE|fetch.*EMBEDDED|net::|EMBEDDED_DATA|localStorage|CORS|favicon/i.test(t)) errs.push('console:' + t.slice(0, 120)); } });
    ...
    const info = await page.evaluate(() => {
      ...
      return { cells, rects: rects.length, areaRatio: +areaRatio.toFixed(2), legends, tip: !!tip };
    });
