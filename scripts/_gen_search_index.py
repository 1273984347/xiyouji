...
block = BEGIN + '\nconst SITE_SEARCH_INDEX = ' + payload + ';\n' + END

targets = [os.path.join(SITE, 'data', 'search.html'), os.path.join(SITE, 'en', 'search.html')]
for t in targets:
    s = rd(t)
...
print(f'docs={len(doc_files)} pages={len(page_files)} bytes={len(payload.encode("utf-8"))}')
print(f'targets=2 embedded (block bytes={len(block.encode("utf-8"))})')
