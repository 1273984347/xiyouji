...
> 三轨并行 CI（screenshots 回归 + lighthouse 性能 + a11y 审查）+ main 推送 CD（GitHub Pages 自动部署）。
...
| Deploy | [`deploy.yml`](deploy.yml) | `push` on `main` | main 合并后自动部署 GitHub Pages（前置 a11y 门禁） |

> 设计原则：PR 走 CI 验证（不部署），main 走 CD 部署（轻量前置门禁 + Pages 上线）。
> CI 与 CD 解耦：PR 阶段三轨并行可在 5–8 分钟内全部跑完，merge 后 CD 仅约 1–2 分钟上线。
...
| 事件 | 目标分支 | 触发工作流 | 三轨 | 部署 |
| --- | --- | --- | --- | --- |
| `pull_request` opened/synchronize/reopened | `main` | `ci.yml` | ✅ 全跑 | ❌ |
| `push` | `main` | `deploy.yml` | ❌（仅前置 a11y） | ✅ Pages |
| `push` 到非 main 分支 | — | ❌ | ❌ | ❌ |
| 手动 `workflow_dispatch` | — | 未启用 | — | — |

> `concurrency` 策略：CI 同 PR 后续 push 取消前次（`cancel-in-progress: true`）；CD 串行不取消（`cancel-in-progress: false`，避免 Pages 部署中断）。
...
| `github-pages` | deploy（系统内置） | `./site` 静态产物 | Pages 托管 |
...
| a11y P0 计数 | = 0 | ci.yml a11y-audit job 失败 / deploy.yml 前置门禁阻断 |
...
| CI/CD 化 | **W234-E1**（本工作流） | 提供 PR 门禁 + main 部署两轨，闭合"开发-验证-上线"循环 |
